...
A: По множеству причин:
  ...
  * нет готовых выверенных шаблонов для мониторинга приложений/сервисов. в стиле prometheus exporter - взял и поехал. все время требуется напильник.
  * Очень сложный механизм LLD, особенно дебаг. 
  * Отсутсвие отказоустойчивости на уровне сервера и отсутствие возможности распределения нагрузки обращения к СУБД из коробки (хочется указать слейвы для чтения и писать в мастер). 
  * Отсутствие гибкой системы полномочий - например нет возможносит давать права на дашборды. нет удобной агрегации в графиках ПРОСТО НЕВОЗМОЖНО в дашборд добавить какое-то нужное поле, а тем более вкинуть туда формулу.
  * Слабая система интеграции с LDAP - по сути есть только проверка пароля для пользователя, но нет полной системы авторизации на основе групп. 
  * ОЧень неудобно переносить данные между ззаббиксами - нет механизма удобного импорта/экспорта.
  * Очень все плохо с повторяемостью и тиражированием - я не могу шаблонизировать конфиги заббикса. Только базовые. А хочу еще права и полномочия,юзеров, дашборды итд итп. То есть пора переходить на yaml а не хранить настройки в БД.
  * СУБД, Все ок пока объектов мало. Как только объектов больше - начинается ад. Обычная СУБД не может выступать в роли TSDB.
  * Микросервисы - Заббикс ну умеет работать с дискавери-сервисами и динамически работать, например с докером.
  * Нет автоматических карт.
  * НЕТ PARENT-CHILD и из-за этого невозможно и карты построить. Рисовать руками принципиально не хочется.
